        ...
        <article class="col-md-8 col-md-push-2"> 
            <div class="panel panel-default">
                <div class="panel-body">
                    <form method="post" action="<?php echo base_url('user/post_artikel_seleksi')."/".$this->uri->segment(3, 0) ?>">
                        <label>Seleksi Artikel</label><br>
                        <?php foreach($artikel as $row_artikel):?>
                        
                        <div class="form-group">
                            <input name="judul" class="form-control" value="<?php echo $row_artikel->judul ?>" required><br>
                            <textarea name="isi-artikel" class="form-control" rows="20" required><?php echo $row_artikel->isi_artikel ?></textarea><br>
                            <button class="btn btn-primary" type="submit">Submit</button>
                            <button class="btn btn-secondary" formaction="<?php echo base_url('user/hapus_artikel')."/".$this->uri->segment(3,0)?>">Hapus</button>
                        </div>
                        <?php endforeach ?>
